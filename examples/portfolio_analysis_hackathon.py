...
# Add the parent (root) directory to sys.path to access custom_utils
...
# Initialize logger
logger = get_logger("PL Analysis")

# read predcited values
pred_path = "../output/pl.csv"
...
nw_ols = sm.ols(formula="port_11 ~ sp_ret", data=monthly_port).fit(
    cov_type="HAC", cov_kwds={"maxlags": 3}, use_t=True
)
logger.info(nw_ols.summary())

# Specifically, the alpha, t-statistic, and Information ratio are:
logger.info(f"CAPM Alpha: {nw_ols.params["Intercept"]}")
logger.info(f"t-statistic: {nw_ols.tvalues["Intercept"]}")
logger.info(f"Information Ratio :{nw_ols.params["Intercept"] / np.sqrt(nw_ols.mse_resid) * np.sqrt(12)}"
...
logger.info(f"Max 1-Month Loss: {max_1m_loss}")
...
logger.info(f"Long Portfolio Turnover {turnover_count(long_positions)}")
logger.info(f"Short Portfolio Turnover : {turnover_count(short_positions)}")
